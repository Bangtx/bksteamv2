...
class BaseModel(Model):
    ...
    @classmethod
    def get_list(cls):
        return list(cls.select().where(cls.active).dicts().order_by(cls.id))
    ...
    @classmethod
    def get_one(cls, id):
        query = cls.select().where(cls.active, cls.id == id).dicts()
        try:
            return query.get()
        except:
            raise HTTPException(
                status_code=404, detail={"msg": f"{cls.__name__} not found"}
            )
