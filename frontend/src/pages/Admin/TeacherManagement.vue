              ...
              .w-full.overflow-hidden.rounded-lg.shadow-xs.mt-4(class='lg:px-10')
                .w-full.overflow-auto.rounded-lg(style='max-height: 600px;')
                  table.w-full.whitespace-nowrap.rounded-lg.border
                    ...
                    tbody.bg-white
                      tr.text-gray-700(v-for="(item, index) in teachers" :key="item.id")
                        ...
                        td.px-4.py-3.text-sm.border
                          .flex.items-center.justify-center.gap-4
                            button.bg-green-600.text-white.px-4.py-2.rounded.mt-6(
                              class='hover:bg-green-500 md:mt-0'
                              @click="curTeacher=item, openTeacherDialog('edit')"
                            )
                              span Sửa
...
const TeacherManagement = defineComponent({
  components: { MenuComponent, HeaderBar, TeacherDialog },
  setup() {
    const { $toast } = getCurrentInstance().proxy
    const teachers = ref([])
    const curTeacher = ref({})
    const isAdd = ref(false)
    const isOpenDialog = ref(false)

    const getData = async () => {
      try {
        const { data } = await api.get(`${endpoints.TEACHER}`)
        teachers.value = data
      } catch {
