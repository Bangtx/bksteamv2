...
const NotificationStudent = defineComponent({
  ...
  setup() {
    ...
    const notificationsSearched = ref([])
    ...
    const getData = async () => {
      try {
        const { data } = await api.get(
          `${endpoints.NOTIFICATION}?class_room=${classroomID}&student_id=${member.id}`
        )
        notifications.value = data
        notificationsSearched.value = notifications.value.filter((e) => e.type === 'progress')
      } catch {
    ...
    const onSelectItem = () => {
      if (selectedItem.value !== 'All') {
        notificationsSearched.value = notifications.value.filter(
          (e) => e.type === selectItems[selectItemsVN.indexOf(selectedItem.value)]
        )
