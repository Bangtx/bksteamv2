        ...
        .px-4.text-gray-700(class='md:px-8')
          ...
          .flex.justify-between.items-center
            ...
            button.flex.items-center.justify-between.px-4.py-2.font-medium.leading-5.text-white.transition-colors.duration-150.bg-orange-400.border.border-transparent.rounded-lg(
              @click="openRegisterSelfLearningDialog(member.type_member)"
            ...
            table.w-full.whitespace-nowrap.rounded-lg.border(
              ...
              tbody.bg-white
                tr.text-gray-700(v-for="(selfLearning, index) in selfLearnings" :key="index")
                  ...
                  td.px-2.py-1.border {{ toAbsentTypeName(selfLearning.absent_type) }}
                  td.px-2.py-1.text-sm.border
                    .flex.items-center.justify-center.gap-4
                      button.bg-red-500.text-white.px-4.py-2.rounded.mt-6.ml-4(
                        class='hover:bg-red-400 md:mt-0'
                        @click="deleteSelfLearning(selfLearning)"
                      )
                        span Xóa
            ...
            table.w-full.whitespace-nowrap.rounded-lg.border.mt-4(
              ...
              tbody.bg-white
                tr.text-gray-700(v-for="(selfL, index) in student.data" :key="index")
                  ...
                  td.px-2.py-1.border(@dblclick="onEditRollCall(selfL.id, student.student.id, selfL.date)") {{ toAbsentTypeName(selfL.absent_type) }}
...
import { EditRollCallDialog, HeaderBar, MenuComponent, RegisterSelfLearningDialog } from '@/components'
...
const RegisterSelfLearning = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onEditRollCall = (id, studentId, rollCallDate) => {
    ...
    const openRegisterSelfLearningDialog = (type_member) => {
      isOpenRegisterSelfLearningDialog.value = true
      dataRegisterProps.value = {
        classroom: classroomID,
        student: type_member === 'teacher' ? null : member.id,
        dates: selfLearnings.value.map((e) => e.date),
        type_member: type_member
      }
    }

    const groupStudent = () => {
      let students = selfLearningAllStudents.value.map((e) => e.student.id)
      students = [...new Set(students)]
      studentData.value = []
      students.forEach((student) => {
        const stu = selfLearningAllStudents.value.filter((e) => e.student.id === student)
      ...
      try {
        if (member.type_member === 'teacher') {
          const { data } = await api.get(
            `${endpoints.SELF_LEARNING}?classroom=${classroomID}`
          )
          selfLearningAllStudents.value = data
          groupStudent()
        } else {
          const { data } = await api.get(
            `${endpoints.SELF_LEARNING}?classroom=${classroomID}&student=${member.id}`
          )
          selfLearnings.value = data
          console.log(selfLearnings.value)
        }
      } catch (e) {
    ...
    const deleteSelfLearning = async (dataDelete) => {
      try {
        const { data } = await api.delete(`${endpoints.SELF_LEARNING}${dataDelete.id}`)
    ...
    const toAbsentTypeName = (absentType) => {
      let ab = null
      if (absentType) {
        ab = absentTypes.find(e => e.id === absentType)
